...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 20 06:04:32 2020
...
for matchNum, match in enumerate(matches, start=1):
    acc.append(match.group())

    #print ("Match {matchNum} was found at {start}-{end}: {match}".format(matchNum = matchNum, start = match.start(), end = match.end(), match = match.group()))

    for groupNum in range(0, len(match.groups())):
        groupNum = groupNum + 1
       # acc.append(match.group(groupNum))
...
df = pd.read_html('/home/snowden/Work/NLTP/data/pgt-depenses-2016.html')
print(df)
ca = pd.read_csv('/home/snowden/Work/NLTP/data/pgt-depenses-2016.csv')

table = pd.DataFrame(ca, index=['date',  'lebelle', 'depense', 'account', 'autre'], columns=acc)
print(table)
